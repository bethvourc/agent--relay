...
def list_v2_session_ids(repo_root: Path) -> list[str]:
    root = sessions_root(repo_root)
    if not root.exists():
        return []
    ids: list[str] = []
    for candidate in root.iterdir():
        if candidate.is_dir() and is_v2_session_dir(candidate):
...
def load_session_manifest(repo_root: Path, session_id: str) -> SessionManifest:
    ...
    if manifest.session_id != session_id:
        raise V2CorruptionError("session manifest session_id does not match directory name", session_id=session_id, path=path)
    return manifest
...
def load_journal_events(repo_root: Path, session_id: str) -> list[JournalEvent]:
    ...
    if not directory.exists():
        raise V2CorruptionError("journal directory is missing", session_id=session_id, path=directory)
    ...
    for event_path in sorted(directory.glob("*.json")):
        try:
            ...
            raise V2CorruptionError(str(exc), session_id=session_id, path=event_path) from exc
    if not events:
...
def _load_session_manifest_path(path: Path, *, session_id: str) -> SessionManifest:
    try:
        ...
        raise V2CorruptionError(str(exc), session_id=session_id, path=path) from exc
    return manifest
...
def _validate_repo_layout_version(repo_root: Path) -> None:
    ...
    if not path.exists():
        raise V2CorruptionError("v2 repo layout is missing VERSION", path=path)
    value = path.read_text(encoding="utf-8").strip()
    if value != LAYOUT_VERSION:
        raise V2CorruptionError(f"unsupported v2 layout version: {value}", path=path)
...
def _load_object_from_ref(session_root_path: Path, ref: ObjectRef):
    ...
    try:
        ...
        raise V2CorruptionError(str(exc), session_id=session_root_path.name, path=manifest_path) from exc
