...
| `agent-relay limits`                    | Show real subscription usage for the agent CLIs (Claude's 5-hour/weekly windows, Codex's quota). Read from each CLI's own source, never estimated; agents that expose no quota say so. `--refresh` re-reads instead of using the short-lived cache.                    |
| `agent-relay handoff plan --agent <agent>` | Who would take over if `<agent>` ran out right now, and why not the others ("codex is rate-limited", "gemini is not installed"). Offline. Prefers a successor already chosen by `handoff arm`; falls back to your `handoff.order`. `--session-id` includes the parked pick, `--prefer` tries one agent first, `--limited` records `<agent>` as out before deciding. |
| `agent-relay handoff arm --agent <agent> --session-id <id>` | Pick a successor now, while `<agent>` still has the quota to answer, and park it on disk. No-ops below `handoff.successor_threshold` (90%). This is what the REPL and the desktop app call after a turn — running it by hand is mostly for debugging. |
